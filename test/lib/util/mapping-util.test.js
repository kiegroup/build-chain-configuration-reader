const {
  getBaseBranch,
  getBaseMappingInfo
} = require("../../../src/lib/util/mapping-util");
...
describe("getBaseMappingInfo", () => {
  ...
  test("targetBranch same. projectB no matching mapping", () => {
    ...
    // Assert
    expect(result).toStrictEqual({ source: "mainx", target: "7.x.y" });
  });
  ...
  test("targetBranch same. regex", () => {
    ...
    // Assert
    expect(result).toStrictEqual({ source: ".*", target: "main.*" });
  });
  ...
  test("targetBranch same. regex2", () => {
    ...
    // Assert
    expect(result).toStrictEqual({ source: "\\d\\..", target: "main.*" });
  });
  describe("exclude", () => {
    const projectTriggeringTheJobMappingExclude = {
      dependencies: {
    ...
    test("dependency default exclude. not matching branch", () => {
      ...
      // Assert
      expect(result).toStrictEqual({ source: "8.x", target: "main" });
    });
  });
});
